...
def getOptimizerGroup(model):
    ...
    optimizerConfig = [{"params": otherParams}, {"params": noDecay, "weight_decay": 0e-7}]
...
def initializeCheckpoint(Model, device, max_lr, weight_decay, nIter, conf):
    ...
    lrScheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer, max_lr, nIter, pct_start=0.05, cycle_momentum=False, final_div_factor=2, div_factor=20
    )
...
def load_checkpoint(Model, conf, filename, device, strict=False):
    ...
    lrScheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer, 4e-4, 500000, pct_start=0.05, cycle_momentum=False, final_div_factor=2, div_factor=20
    )
